...
class JackCompilationEngine: 
    ...
    def compile_subroutine(self, start_token): 
            ...
            print("compiling subroutine - " + start_token)
            ...
            print("data type - " + self.tk.current_token)            
            if self.tk.token_type(self.tk.current_token) == "IDENTIFIER": 
                self.output_buffer.append("<identifier> " + current_token + " </identifier>")
            elif self.tk.token_type(self.tk.current_token) == "KEYWORD": 
            ...
            self.tk.advance()
            current_token = self.tk.get_token()
            print("routine name - " + self.tk.current_token)
    ...
    def compile_statements(self, start_token):
        ...
        while self.tk.has_more_tokens() and current_token in ('let','if','do','while','return'): 
    ...
    def compile_let(self, start_token): 
        ...
        if current_token == "[": 
            ...
            print("post [ expression " + self.tk.get_token() )
            if not current_token == "]": 
        ...
        print("post expression token in let: " + current_token)
        ###############################################
    ...
    def compile_return(self, start_token): 
        ...
        if not (self.tk.current_token == ";"): 
            ...
            print("return token " + self.tk.current_token )
            self.output_buffer.append("<symbol> " + self.tk.current_token + " </symbol>")
        else:    
        ...
        self.output_buffer.append("</returnStatement>")
        print("final return token:" + self.tk.current_token) 
    ...
    def compile_if(self, start_token): 
        ...
        print("pre if expression token" + self.tk.get_token())

        #EXPRESSION
        self.compile_expression(current_token)
        ###############################################

        print(self.tk.get_token())
        if not (self.tk.get_token() == ")"): 
            raise Exception (") expected for if expression")
        self.output_buffer.append("<symbol> " + self.tk.get_token() + " </symbol>")  
        ...
        self.compile_statements(self.tk.get_token())
        current_token = self.tk.get_token() 
    ...
    def compile_expression(self, start_token): 
        ...
        print("expression start while loop token: " + self.tk.current_token)


        #loop for all non terms
        while(self.tk.has_more_tokens and self.tk.current_token in ".+-*/&|<>="): 
            
            if not self.math_transform.get(self.tk.current_token) == None: 
            ...
            print("in loop token = " + self.tk.current_token)

        print("Exit expression final token: " + self.tk.current_token)
    ...
    def compile_term(self): 
        ...
        if self.tk.token_type(self.tk.current_token) == "STRING_CONST": 
            ...
            if self.tk.current_token == "(": 
                ...
                if self.tk.current_token != ')':
                    raise Exception(") expected in return from term")  
                self.output_buffer.append("<symbol> " + self.tk.current_token + " </symbol>")                             

            elif self.tk.current_token in ["-","~"]: 
                print("entering negative / not term block")
                self.output_buffer.append("<symbol> " + self.tk.current_token + " </symbol>")
                self.tk.advance()
                self.compile_term()
                self.output_buffer.append("</term>")
            ...
            if look_ahead == "(": 
                ...
                print("Calling expression from [ " + self.tk.current_token)
                
                self.compile_expression(self.tk.current_token)

                print("After expression from [ " + self.tk.current_token)
                if not self.tk.current_token == ']':
                    raise Exception("Missing ] in array element from term")
                self.output_buffer.append("<symbol> " + self.tk.current_token + " </symbol>")                      

            elif look_ahead == ".":
                print("enter . lookahead")
                self.tk.advance()
                ...
                if not self.tk.token_type(self.tk.current_token) == "IDENTIFIER": 
                    raise Exception("Identifier expected in . let statement")
                print("next identifier: " + self.tk.current_token)
                self.output_buffer.append("<identifier> " + self.tk.current_token + " </identifier>")
                self.tk.advance()
                self.output_buffer.append("<symbol> " + self.tk.current_token + " </symbol>")   
                self.tk.advance()
                 
                print("compile expression list - " + self.tk.current_token)
